...
**プロセス内の初回推論は約 40 秒**を GPU カーネルの特殊化に費やします(主に int8 `lm_head` の 262K-way
argmax GEMM)。これはプロセス毎で、実行間でキャッシュされません。CLI は `[warming up…]` を表示するので
何をしているか分かります。同一セッションの 2 ターン目以降は高速です。
...
- **KV 永続化** *(ライブラリ機能 — この CLI からは使えません)*。`CoreMLChainV2` は KV `MLState` を
  ...
  「長文は初回だけ cold prefill」を成立させる機構です。ライブラリのテストで検証済みですが、`corellm-chat`
  のフラグとしては露出していません — API の部品として扱ってください。
...
広コンテキストの「固定幅税」は int4 で **3.24×**。ラダーの目的は、必要になるまでこの税を*払わない*ことです。
...
- **128K モードは設計上遅い。** 広グラフは ~300 ms/tok(~3.3 tok/s)で、32K モードの ~90 ms/tok に対し
  int4 で固定 3.24× の税。ラダーで 32K 以下の会話では回避できますが、本当に 128K 深いコンテキストは速く
  ありません。
...
- **初回推論に ~40 秒。** 一度きり・プロセス毎の GPU カーネル特殊化(キャッシュされません)。

---

## トラブルシューティング

- **「最初のトークンで 40 秒固まる」。** 仕様です — 一度きりの `lm_head` GPU 特殊化。同一セッションの warm
  ターンは高速です。ライブラリ組み込み時はアプリ起動時にプリウォームしてください。
...
- **モデル重み**(Hugging Face で別途配布): MIT ではなく
  [Gemma Terms of Use](https://ai.google.dev/gemma/terms) に従います。**Built with Gemma.**

本プロジェクトは独立した非公式のものです。Google・Apple とは提携・承認・後援の関係に**ありません**。
