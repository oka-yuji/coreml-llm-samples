...
2. **GPU 特殊化。** 続く**プロセス内の初回推論が約 40 秒**を GPU カーネルの特殊化に費やします(主に int8
   `lm_head` の 262K-way argmax GEMM)。こちらはプロセス毎で、実行間でキャッシュ*されません*。

CLI は `[warming up…]` を表示するので何をしているか分かります。同一セッションの 2 ターン目以降は高速で、
2 回目以降のプロセス起動ではコンパイルは省かれます(GPU 特殊化のみ再発)。
